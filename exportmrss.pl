...
my $sth = $dbh->prepare("select b.filename, b.name, b.description, c.name as cname, t1ts-t0ts as duration, c.id as cid, b.t0ts, b.id, b.pid, b.height, b.width from broadcasts b, channels c where "
...
while(my $result = $sth->fetchrow_hashref()) {
    ...
    print '<media:content url="'.$result->{'filename'}
